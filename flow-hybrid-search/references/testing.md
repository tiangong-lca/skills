...
```bash
node scripts/run-flow-hybrid-search.mjs --published-cli --dry-run --json
```
...
```bash
pnpm dlx --package=@tiangong-lca/cli@0.1.12 tiangong-lca auth doctor-auth --json
node scripts/run-flow-hybrid-search.mjs --published-cli --json
```

## Direct CLI equivalent

```bash
pnpm dlx --package=@tiangong-lca/cli@0.1.12 tiangong-lca search flow --input ./assets/example-request.json --dry-run --json
```
