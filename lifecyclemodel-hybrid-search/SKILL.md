...
4. The Node wrapper delegates to `tiangong search lifecyclemodel`; `scripts/run-lifecyclemodel-hybrid-search.sh` is only a compatibility launcher.
5. Confirm response shape, then debug with focused references.

## Commands
```bash
TIANGONG_LCA_API_KEY="<your-api-key>" node scripts/run-lifecyclemodel-hybrid-search.mjs --dry-run
TIANGONG_LCA_API_KEY="<your-api-key>" node scripts/run-lifecyclemodel-hybrid-search.mjs
node scripts/run-lifecyclemodel-hybrid-search.mjs --input ./assets/example-request.json --api-key "$TIANGONG_LCA_API_KEY"
TIANGONG_LCA_CLI_DIR=/path/to/tiangong-lca-cli node scripts/run-lifecyclemodel-hybrid-search.mjs --dry-run --api-key "$TIANGONG_LCA_API_KEY"
```
