...
```bash
node scripts/run-process-hybrid-search.mjs --published-cli --dry-run --json
```
...
```bash
pnpm dlx --package=@tiangong-lca/cli@0.1.12 tiangong-lca search process --input ./assets/example-request.json --dry-run --json
```
