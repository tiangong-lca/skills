...
4. The Node wrapper delegates to `tiangong admin embedding-run`; `scripts/run-embedding-ft.sh` is only a compatibility launcher.
...
node scripts/run-embedding-ft.mjs --input ./assets/example-jobs.json --api-key "$TIANGONG_LCA_API_KEY"
TIANGONG_LCA_CLI_DIR=/path/to/tiangong-lca-cli node scripts/run-embedding-ft.mjs --dry-run --api-key "$TIANGONG_LCA_API_KEY"
```
