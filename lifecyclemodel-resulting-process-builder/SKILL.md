...
The active runtime path is `skill -> Node wrapper -> tiangong CLI`. Python and MCP are no longer part of the normal execution path for this skill. The `.sh` file remains only as a compatibility exec shim around the Node wrapper.
...
Canonical request files should use `process_sources.allow_remote_lookup`. Keep it `false` for normal local-only runs; set it to `true` only when you want the CLI to deterministically backfill missing process datasets through its direct Supabase REST read path derived from `TIANGONG_LCA_API_BASE_URL`.
...
Use `--model-file` only when local process sources can be inferred from the model location. Use `--request` to pin `process_sources.*` explicitly. The wrapper keeps `--request` and `--model-file` as compatibility flags, but the underlying CLI contract is `--input <request.json>` for build and `--run-dir <dir>` for publish. The legacy shell entrypoint still works, but it only `exec`s the Node wrapper.
