...
- `external-dataset-curated-import`, `source-evidence-dataset-development`, `dataset-rls-maintenance`, and `lca-topic-overview` are top-level workflow skills only; executable conversion, queue state, validation, QA, write/delete/redo, verify, public inventory/statistics/relations and rendering stay in CLI/Foundry-owned commands. Topic overview uses a capability-checked explicitly selected CLI; its unreleased companion protocol does not advance or bypass the shared 0.1.8 wrapper pin.
